...
export default function ExamDetailDisplay() {
  ...
  const { data, loading, error } = useFetch(examId ? () => examApi.getWithResult(Number(examId)) : null, [examId])
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto border rounded-lg shadow">
      ...
      {result ? (
        <p className="text-green-600 font-semibold mb-4">Your Score: {result.score}</p>
      ) : (
